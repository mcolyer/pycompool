...
class TestPoolController:
    ...
    @patch("pycompool.controller.SerialConnection")
    def test_aux_equipment_no_op_when_already_in_desired_state(
...
class TestServiceModeProtection:
    ...
    @patch("pycompool.controller.SerialConnection")
    def test_pool_temperature_blocked_in_service_mode(
        self, mock_connection_class, capsys
    ):
        ...
        with patch.object(PoolController, "get_status") as mock_status:
            ...
            captured = capsys.readouterr()
            assert (
                "Service mode is active - commands are disabled for safety"
                in captured.out
            )

    @patch("pycompool.controller.SerialConnection")
    def test_spa_temperature_blocked_in_service_mode(
        self, mock_connection_class, capsys
    ):
