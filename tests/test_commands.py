...
class TestCommands:
    ...
    def test_set_pool_command_with_options(self, mock_controller_class):
        ...
        result = set_pool_command("80f", port="/dev/ttyUSB1", baud=19200, verbose=True)
    ...
    def test_set_spa_command_with_options(self, mock_controller_class):
        ...
        result = set_spa_command(
            "104f", port="socket://192.168.1.50:8899", baud=9600, verbose=True
        )

        assert result is True
        mock_controller_class.assert_called_once_with(
            "socket://192.168.1.50:8899", 9600
        )
    ...
    def test_set_heater_command_with_options(self, mock_controller_class):
        ...
        result = set_heater_command(
            "solar-only", "spa", port="/dev/ttyUSB1", baud=19200, verbose=True
        )

        assert result is True
        mock_controller_class.assert_called_once_with("/dev/ttyUSB1", 19200)
        mock_controller.set_heater_mode.assert_called_once_with(
            "solar-only", "spa", True
        )
    ...
    def test_set_aux_command_with_options(self, mock_controller_class):
        ...
        result = set_aux_command(
            "aux2", "off", port="/dev/ttyUSB1", baud=19200, verbose=True
        )
    ...
    def test_set_aux_command_invalid_aux_name(self):
        ...
        with pytest.raises(
            ValueError, match="Invalid aux name 'invalid'. Must be aux1, aux2, etc."
        ):
            set_aux_command("invalid", "on")

        with pytest.raises(
            ValueError, match="Invalid aux name 'aux'. Must be aux1, aux2, etc."
        ):
            set_aux_command("aux", "on")

        with pytest.raises(
            ValueError, match="Invalid aux name 'auxX'. Must be aux1, aux2, etc."
        ):
    ...
    def test_set_aux_command_invalid_state(self):
        ...
        with pytest.raises(
            ValueError, match="Invalid state 'invalid'. Must be 'on' or 'off'."
        ):
            set_aux_command("aux1", "invalid")

        with pytest.raises(
            ValueError, match="Invalid state 'true'. Must be 'on' or 'off'."
        ):
