...
class TestPoolMonitor:
    ...
    def test_start_calls_monitor_loop(self, mock_connection_class, mock_signal):
        ...
        mock_connection.read_packets.assert_called_once_with(
            packet_size=24, timeout=30.0
        )
    ...
    def test_display_heartbeat_basic(self, capsys):
        ...
        assert (
            "[HEAT/AUX1/AUX2]" in captured.out
        )  # Status flags including aux equipment
    ...
    @patch("pycompool.monitor.SerialConnection")
    @patch("pycompool.monitor.signal")
    def test_monitor_loop_packet_processing(
        self, mock_signal, mock_connection_class, mock_parse, capsys
    ):
        ...
        with patch(
            "pycompool.monitor.time.time", side_effect=[0, 1, 2, 15]
        ):  # Trigger timeout
            with patch("pycompool.monitor.time.strftime", return_value="14:30:15"):
    ...
    @patch("pycompool.monitor.SerialConnection")
    @patch("pycompool.monitor.signal")
    def test_monitor_loop_parse_failure_verbose(
        self, mock_signal, mock_connection_class, mock_parse, capsys
    ):
...
class TestMonitorCommand:
    ...
    def test_monitor_command_with_options(self, mock_monitor_class):
        ...
        monitor_command(port="/dev/ttyUSB1", baud=19200, verbose=True)
